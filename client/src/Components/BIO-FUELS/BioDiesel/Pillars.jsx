import React from "react";
import { ArrowRight, Leaf, Factory, Wind, Droplets, Zap } from "lucide-react";
import { motion } from "framer-motion";

export default function BioDieselPillars() {

  const pillars = [
    {
      title: "Feedstock Pre-treatment",
      category: "VEGETABLE OIL · UCO · ANIMAL FAT",
      badgeTitle: "Flexible",
      badgeSub: "Feedstock Handling",
      desc: "Advanced filtration, degumming, and pre-treatment systems for processing diverse feedstocks like used cooking oil, vegetable oils, and animal fats.",
      img: "https://5.imimg.com/data5/SELLER/Default/2023/5/310546029/EX/FH/JV/187561863/20180427061720-500x500.jpg",
      span: "lg:col-span-2",
      height: "h-[400px] lg:h-[500px]",
      icon: Leaf,
    },
    {
      title: "Transesterification Process",
      category: "CHEMICAL · REACTION · CATALYST",
      badgeTitle: "High Efficiency",
      badgeSub: "Fuel Conversion",
      desc: "Optimized chemical conversion process using catalysts to transform oils and fats into high-quality biodiesel and glycerin.",
      img: "https://www.shutterstock.com/image-photo/hand-biofuel-refueling-nozzle-on-600nw-2495080315.jpg",
      span: "lg:col-span-1",
      height: "h-[400px] lg:h-[500px]",
      icon: Factory,
    },
    {
      title: "Separation & Purification",
      category: "WASHING · DRYING · REFINING",
      badgeTitle: "99%",
      badgeSub: "Fuel Purity",
      desc: "Efficient separation of biodiesel from glycerin followed by washing and drying to achieve fuel-grade purity standards.",
      img: "https://cdn.prod.website-files.com/6320bb946c79f421bd3b702f/6345990067d0362acd058742_Chemsitry.jpeg",
      span: "lg:col-span-1",
      height: "h-[360px]",
      icon: Wind,
    },
    {
      title: "By-product Management",
      category: "GLYCERIN · RECOVERY · UTILIZATION",
      badgeTitle: "Sustainable",
      badgeSub: "Zero Waste",
      desc: "Recovery and refinement of glycerin and other by-products, ensuring maximum resource utilization and minimal waste generation.",
      img: "https://thumbs.dreamstime.com/b/ikea-commercial-warehouse-interior-distribution-storage-logistics-copenhagen-denmark-january-434743550.jpg",
      span: "lg:col-span-1",
      height: "h-[360px]",
      icon: Droplets,
    },
    {
      title: "Storage & Distribution",
      category: "TANKS · LOGISTICS · FUEL SUPPLY",
      badgeTitle: "Ready",
      badgeSub: "Market Supply",
      desc: "Integrated storage, handling, and distribution systems for efficient delivery of biodiesel to transportation and industrial sectors.",
      img: "https://www.stocklogistic.com/wp-content/themes/electronaut/img/valencia.jpg",
      span: "lg:col-span-1",
      height: "h-[360px]",
      icon: Zap,
    },
  ];

  return (
    <section className="py-24 px-8 max-w-[90rem] mx-auto w-full bg-[var(--color-bg)]">
      
      <motion.div
        initial={{ opacity: 0, y: 40 }}
        whileInView={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.7 }}
        viewport={{ once: true }}
        className="mb-16 grid gap-8 lg:grid-cols-2"
      >
        <div>
          <div className="flex items-center gap-4 mb-6">
            <div className="w-8 h-[1px] bg-[var(--color-secondary)]"></div>
            <span className="text-[var(--color-secondary)] text-[10px] font-bold tracking-[0.2em] uppercase">
              Our Specialisations
            </span>
          </div>

          <h2 className="font-serif text-4xl md:text-5xl lg:text-[3.5rem] text-[var(--color-text)] leading-[1.1]">
            Core Capabilities in <br />
            <span className="italic text-[var(--color-secondary)]">Biodiesel</span> EPC
          </h2>
        </div>

        <div className="flex items-end">
          <p className="text-[var(--color-muted)] max-w-lg text-lg">
            Delivering complete infrastructure for renewable liquid fuel production — from oil-based feedstock processing to high-quality biodiesel refining and distribution.
          </p>
        </div>
      </motion.div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {pillars.map((pillar, idx) => {
          const Icon = pillar.icon;
          
          return (
            <motion.div
              key={idx}
              initial={{ opacity: 0, y: 60 }}
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ delay: idx * 0.1, duration: 0.6 }}
              viewport={{ once: true }}
              whileHover={{ y: -8 }}
              className={`group relative overflow-hidden rounded-xl bg-[var(--color-bg-alt)] border border-[var(--color-border)] shadow-sm hover:shadow-xl transition-all duration-500 ${pillar.span} ${pillar.height}`}
            >
              <motion.img
                src={pillar.img}
                alt={pillar.title}
                className="absolute inset-0 w-full h-full object-cover opacity-60 grayscale-[20%] group-hover:grayscale-0 transition-all duration-700"
                whileHover={{ scale: 1.05 }}
              />

              <div className="absolute inset-0 bg-gradient-to-t from-[var(--color-bg-deep)] via-[var(--color-bg-deep)]/80 to-transparent group-hover:via-[var(--color-bg-deep)]/60 transition-all duration-500"></div>

              <div className="absolute top-6 left-6 right-6 flex justify-between items-start">
                <div className="bg-[var(--color-bg-deep)]/40 backdrop-blur-md p-3 rounded-lg border border-[var(--color-border)]/50 text-[var(--color-secondary)]">
                  <Icon size={24} strokeWidth={1.5} />
                </div>
                
                <div className="backdrop-blur-md bg-[var(--color-bg)]/60 border border-[var(--color-border)]/50 px-5 py-2.5 rounded-lg text-right group-hover:shadow-[0_0_20px_var(--color-secondary)] group-hover:border-[var(--color-secondary)]/50 transition-all duration-500">
                  <span className="block font-serif text-2xl text-[var(--color-text)] leading-none">
                    {pillar.badgeTitle}
                  </span>
                  <span className="block text-[10px] text-[var(--color-text-muted)] mt-1.5 uppercase tracking-wider font-medium">
                    {pillar.badgeSub}
                  </span>
                </div>
              </div>

              <div className="absolute bottom-0 p-8 md:p-10 translate-y-6 group-hover:translate-y-0 transition-all duration-500 w-full">
                
                <span className="text-[10px] font-bold tracking-[0.15em] text-[var(--color-secondary)] mb-3 block">
                  {pillar.category}
                </span>

                <h3 className="font-serif text-3xl text-[var(--color-text)] mb-3 drop-shadow-md">
                  {pillar.title}
                </h3>

                <p className="text-sm text-[var(--color-text-muted)] opacity-0 group-hover:opacity-100 transition-opacity duration-500 mb-6 max-w-[90%] leading-relaxed">
                  {pillar.desc}
                </p>

                <motion.a
                  href="#contact"
                  whileHover={{ x: 5 }}
                  className="inline-flex items-center gap-2 text-xs uppercase font-bold tracking-wider text-[var(--color-text)] border-b border-[var(--color-border-muted)] hover:border-[var(--color-secondary)] hover:text-[var(--color-secondary)] pb-1 transition-colors"
                >
                  Learn More
                  <ArrowRight size={16} />
                </motion.a>
              </div>
            </motion.div>
          );
        })}
      </div>
    </section>
  );
}